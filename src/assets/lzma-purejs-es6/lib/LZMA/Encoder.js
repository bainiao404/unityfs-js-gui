...
import RangeCoder from '../RangeCoder.js'
...
Encoder.prototype.getRepLen1Price = function (state, posState) {
    return (
        RangeCoder.Encoder.getPrice0(this._isRepG0[state]) +
...
Encoder.prototype.getOptimum = function (position) {
    ...
    this._optimum[1].price =
        RangeCoder.Encoder.getPrice0(this._isMatch[(this._state << Base.kNumPosStatesBitsMax) + posState]) +
        this._literalEncoder
            .getSubCoder(position, this._previousByte)
            .getPrice(!Base.stateIsCharState(this._state), matchByte, currentByte)
    ...
    while (true) {
        ...
        matchPrice =
            curPrice + RangeCoder.Encoder.getPrice1(this._isMatch[(state << Base.kNumPosStatesBitsMax) + posState])
            ...
            if (lenTest < numAvailableBytesFull) {
                ...
                if (lenTest25 >= 2) {
                    ...
                    var curAndLenCharPrice =
                        ...
                        RangeCoder.Encoder.getPrice0(
                            this._isMatch[(state25 << Base.kNumPosStatesBitsMax) + posStateNext5],
                        ) +
                        this._literalEncoder
                            .getSubCoder(position + lenTest, this._matchFinder.getIndexByte(lenTest - 2))
                            .getPrice(
                                true,
                ...
                if (lenTest6 === this._matchDistances[offs6]) {
                    if (lenTest6 < numAvailableBytesFull) {
                        ...
                        if (lenTest27 >= 2) {
                            ...
                            var curAndLenCharPrice7 =
                                curAndLenPrice6 +
                                RangeCoder.Encoder.getPrice0(
                                    this._isMatch[(state27 << Base.kNumPosStatesBitsMax) + posStateNext7],
                                ) +
                                this._literalEncoder
                                    .getSubCoder(position + lenTest6, this._matchFinder.getIndexByte(lenTest6 - 2))
                                    .getPrice(
                                        true,
                            ...
                            var nextRepMatchPrice7 =
                                nextMatchPrice7 + RangeCoder.Encoder.getPrice1(this._isRep[state27])
                            ...
                            var curAndLenPrice7 =
                                nextRepMatchPrice7 + this.getRepPrice(0, lenTest27, state27, posStateNext7)
...
Encoder.prototype.fillDistancesPrices = function () {
    ...
    for (; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
        ...
        for (posSlot = Base.kEndPosModelIndex; posSlot < this._distTableSize; posSlot++) {
            this._posSlotPrices[st + posSlot] +=
                ((posSlot >>> 1) - 1 - Base.kNumAlignBits) << RangeCoder.Encoder.kNumBitPriceShiftBits
        }
