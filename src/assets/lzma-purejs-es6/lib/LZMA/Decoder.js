...
Decoder.prototype.code = function (inStream, outStream, outSize) {
    ...
    while (outSize < 0 || nowPos64 < outSize) {
        ...
        if (
            this._rangeDecoder.decodeBit(this._isMatchDecoders, (state << Base.kNumPosStatesBitsMax) + posState) === 0
        ) {
            ...
            if (this._rangeDecoder.decodeBit(this._isRepDecoders, state) === 1) {
                ...
                if (posSlot >= Base.kStartPosModelIndex) {
                    ...
                    if (posSlot < Base.kEndPosModelIndex) {
                        ...
                        rep0 +=
                            this._rangeDecoder.decodeDirectBits(numDirectBits - Base.kNumAlignBits) <<
                            Base.kNumAlignBits
...
Decoder.prototype.setDecoderProperties = function (properties) {
    ...
    dictionarySize = 0
    for (i = 0, shift = 1; i < 4; i++, shift *= 256) dictionarySize += (properties[1 + i] & 0xff) * shift
