...
describe('range coder', function () {
    ...
    it('encodeDirectBits->decodeDirectBits should be the identity', function () {
        ...
        enc.flushData()
        // show output
        if (DEBUG) console.log(stream.data.length, Buffer.from(stream.data).toString('hex'))
        // now decode this.
    ...
    var testDecodeBit = function (probsLength, makeContext, name) {
        ...
        // show output
        if (DEBUG) console.log(name || '', stream.data.length, Buffer.from(stream.data).toString('hex'))
    ...
    for (contextLength = 1; contextLength <= 16; contextLength++) {
        it('encode->decodeBit (' + contextLength + '-bit context)', makeRunningContextTest(contextLength))
    }
